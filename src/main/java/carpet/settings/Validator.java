...
public abstract class Validator<T> {
    ...
    public static class _CLIENT<T> extends Validator<T> {
        ...
        public String description() {
            return "Its a client command so can be issued and potentially be effective when connecting to non-carpet/vanilla servers. " +
    ...
    public static class _COMMAND_LEVEL_VALIDATOR extends Validator<String> {
        private static final ImmutableList<String> OPTIONS = ImmutableList.of("true", "false", "ops", "0", "1", "2", "3", "4");
        @Override public String validate(CommandSource source, ParsedRule<String> currentRule, String newValue, String userString) {
    ...
    public static class _STRICT_IGNORECASE<T> extends Validator<T>
    {
        ...
        public T validate(CommandSource source, ParsedRule<T> currentRule, T newValue, String string) {
            if (!currentRule.options.stream().map(s->s.toLowerCase(Locale.ROOT)).collect(Collectors.toSet())
                ...
                Messenger.m(source, "r Valid options (case insensitive): " + currentRule.options);
                return null;
            }
            return newValue;
        }
