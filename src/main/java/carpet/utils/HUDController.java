...
public class HUDController {
    ...
    public static void update_hud(MinecraftServer server) {
        ...
        for (EntityPlayer player: player_huds.keySet()) {
            SPacketPlayerListHeaderFooter packet = new SPacketPlayerListHeaderFooter();
            ((SPacketPlayerListHeaderFooterMixin)packet).setHeader(new TextComponentString(""));
            ((SPacketPlayerListHeaderFooterMixin)packet).setFooter(Messenger.c(player_huds.get(player).toArray(new Object[0])));
            ((EntityPlayerMP)player).connection.sendPacket(packet);
        }
    }
    ...
    private static ITextComponent [] send_tps_display(MinecraftServer server) {
        ...
        return new ITextComponent[]{Messenger.c("g TPS: ", String.format(Locale.US, "%s %.1f",color, TPS), "g  MSPT: ", String.format(Locale.US,"%s %.1f", color, MSPT))};
    }
    ...
    private static ITextComponent[] send_counter_info(MinecraftServer server, String color) {
        ...
        return res.toArray(new ITextComponent[0]);
    }
